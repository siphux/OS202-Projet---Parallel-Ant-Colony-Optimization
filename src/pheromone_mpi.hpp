...
public:
    ...
    int data_size() const {
        ...
        unsigned long get_dim() const { return m_dim; }
        unsigned long get_stride() const { return m_stride; }
    ...
    void do_evaporation_mpi(int start_row, int end_row) {
    ...
    void sync_evaporation_mpi(int start_row, int local_rows, int size, int rank) {
        std::vector<int> recvcounts(size);
        std::vector<int> displs(size);

        int elements_per_row = m_stride * 2; 

        for (int i = 0; i < size; ++i) {
            int r_per_proc = m_dim / size;
            int rem = m_dim % size;
            int l_rows = r_per_proc + (i < rem ? 1 : 0);
            int s_row = 1 + i * r_per_proc + std::min(i, rem);
            
            recvcounts[i] = l_rows * elements_per_row;
            displs[i] = s_row * elements_per_row;
        }

        MPI_Allgatherv(MPI_IN_PLACE, 0, MPI_DATATYPE_NULL, 
                    data_buffer(), recvcounts.data(), displs.data(), 
